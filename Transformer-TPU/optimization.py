...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu, scope=None, update_freq=1):
  ...
  global_steps_float = global_steps_float / float(update_freq)

  decay = tf.minimum((global_steps_float + 1) * warmup_steps_float ** -1.5, (global_steps_float + 1) ** -0.5)
  decay = tf.minimum(decay, 4000 * (4000 ** -1.5))
  decay = 10.0 * (256 ** -0.5) * decay
